...
###
Gitters2=as.data.frame(cbind(Salary=y,x))
lfit <- lm(Salary ~ ., data = Gitters2[-testid, ])
lpred <- predict(lfit, Gitters2[testid, ])
with(Gitters2[testid, ], mean(abs(lpred - Salary)))
###

library(nnet)
nnet1=nnet(Salary ~ ., data = Gitters2[-testid, ],size = 2, rang = 0.1,
           decay = 5e-4, maxit = 200)
methods(class = class(nnet1)[2])
summary(nnet1)
coef(nnet1)
coef(lfit)
...
###
library(neuralnet)
net1=neuralnet(Salary ~ ., data = Gitters2[-testid, ],hidden = 0,act.fct=function(x){x})
methods(class = class(net1))
plot(net1)
coef(lfit)
net1$weights[[1]][[1]]
gwplot(net1)
gwplot(net1,selected.covariate = "Hits")
gwplot(net1,selected.covariate = "Walks")
gwplot(net1,selected.covariate = "Assists")
...
net2=neuralnet(Salary ~ ., data = Gitters2[-testid, ],hidden = 10,act.fct=function(x){x})
plot(net2)
plotnet(net2)

net3=neuralnet(Salary ~ ., data = Gitters2[-testid, ],hidden = c(10,10),act.fct=function(x){x})
plot(net3)
plotnet(net3)
...
gwplot(net1,selected.covariate = "Assists")
gwplot(net2,selected.covariate = "Assists")
gwplot(net3,selected.covariate = "Assists")

net4=neuralnet(Salary ~ ., data = Gitters2[-testid, ],hidden = 50,act.fct=function(x){ifelse(x>0,x,0)})
plot(net4)
plotnet(net4)
plotmo(net4)
###
...
library(zeallot) # for Multiple assignment operators %<-%
...
save(train_ds,test_ds,file = "Rdata/mnist_array.Rdata")
load("Rdata/mnist_array.Rdata")
...
contour(1:28,1:28,t(train_ds[1]$x)[,28:1],asp=1)
contour(1:28,1:28,t(test_ds[2]$x)[,28:1],asp=1)

library(EBImage)
par(mfrow=c(2,2))
display(1-t(train_ds[1]$x),method = "raster")
title(train_ds[1]$y-1)
display(1-t(test_ds[2]$x),method = "raster")
title(test_ds[2]$y-1)
display(1-t(test_ds[3]$x),method = "raster")
title(test_ds[3]$y-1)
display(1-t(test_ds[1]$x),method = "raster")
title(test_ds[1]$y-1)
par(mfrow=c(1,1))
...
par(mfrow=c(r,c))
for(i in 1:(r*c)){
  display(1-t(train_ds[i]$x),method = "raster")
  title(train_ds[i]$y-1)
}
for(i in 1:(r*c)){
  display(1-t(test_ds[i]$x),method = "raster")
  title(test_ds[i]$y-1)
}
par(mfrow=c(1,1))
...
save(train_ds,test_ds,file = "Rdata/mnist_tensor.Rdata")
load("Rdata/mnist_tensor.Rdata")
...
table(yhat-1,truth-1)
library(caret)
confusionMatrix(as.factor(yhat-1),as.factor(truth-1))

library(ggplot2)
ggplot(mapping = aes(x=as.factor(truth-1),y=as.factor(yhat-1)))+
  geom_count()
...
save(train_ds,test_ds,file = "Rdata/cifar100.Rdata")
load("Rdata/cifar100.Rdata")
...
f=function(x){
  ...
  g=t(x[2,,])
  b=t(x[3,,])
  y=abind(r,g,b,along=3)
  Image(y,colormode="color")
}
y=f(x=train_ds[1]$x)
dim(y)
...
table(yhat,truth)
library(caret)
confusionMatrix(as.factor(yhat),as.factor(truth))

library(ggplot2)
ggplot(mapping = aes(x=truth,y=yhat))+
  geom_count()
###
...
for (i in 1:num_images) {
  ...
  x[i,,, ] <- img
  display(f(img), method = "raster", all = TRUE)
}
par(mfrow=c(1,1))
###
...
save(mapping,file = "Rdata/mapping.Rdata")
load("Rdata/mapping.Rdata")
...
save(imdb_train,imdb_test,file = "Rdata/imdb.Rdata")
...
save(imdb_train,imdb_test,file = "Rdata/imdb_maxlen.Rdata")
load("Rdata/imdb_maxlen.Rdata")
...
library(ggplot2)
ggplot(mapping = aes(x=as.factor(truth),y=as.factor(yhat)))+
  geom_count()
