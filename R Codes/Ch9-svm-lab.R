...
### Methods for Support Vector Classifier ------
...
conf=confusionMatrix(data=ypred, reference=testdat$y)
conf
...
conf=confusionMatrix(
  reference = dat[-train, "y"],
  data = predict(tune.out$best.model, newdata = dat[-train, ])
)
conf
# Other Kernels:
svmfit <- svm(y ~ ., data = dat[train, ], kernel = "polynomial",  
              gamma = 1, degree = 3, coef0 = 0, cost = 1)
plot(svmfit, dat[train, ])
svmfit <- svm(y ~ ., data = dat[train, ], kernel = "sigmoid",  
              gamma = 1, coef0 = 0, cost = 1)
plot(svmfit, dat[train, ])
...
### Methods for ROC Curves --------------------

library(pROC)
roc_obj=roc(response=dat[-train, "y"],predictor=-fitted)
roc_obj
plot(roc_obj)
pROC::auc(roc_obj)
...
### Methods for SVM with Multiple Classes -----

library(plotmo)
par(mfrow=c(1,2))
plotmo(svmfit, do.par=F)
par(mfrow=c(1,1))
...
# Both arguments factor with same levels
# ?caret::confusionMatrix
conf=caret::confusionMatrix(data=svmfit$fitted, reference=dat$y)
conf

fitted <- attributes(
  predict(svmfit, dat, decision.values = T)
)$decision.values
fitted=cbind(dat,fitted,fit=svmfit$fitted)
head(fitted)
tail(fitted)
...
### Methods for Application to Gene Expression Data ----

# confusionMatrix
library(caret)
# Both arguments factor with same levels
# ?caret::confusionMatrix
caret::confusionMatrix(out$fitted, dat$y)
caret::confusionMatrix(pred.te, dat.te$y)
