...
decision=ifelse(p.values <= .05, "Reject H0", "Do not reject H0")
###
table(decision,
...
library(ggstats)
a2 <- lm(returns ~ manager)
ggcoef_model(a2)
ggcoef_model(a2, add_pairwise_contrasts=T)

library(multcomp)
tuk <- glht(a1, linfct=mcp(manager="Tukey"))
summary(tuk)
plot(tuk)
...
library(ggplot2)
ggplot(data=a1$model, aes(x=manager, y=returns)) +
